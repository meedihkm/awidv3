...
router.get("/", authenticate, async (req, res) => {
  try {
    ...
    const orders = await recurringService.getRecurringOrders(
      req.user.id,
...
router.get("/:id", authenticate, validateUUID("id"), async (req, res) => {
  try {
    const order = await recurringService.getRecurringOrderById(
      req.params.id,
      req.user.organization_id,
    );
...
router.post("/", authenticate, async (req, res) => {
  try {
    ...
    if (!frequency || !["daily", "weekly", "monthly"].includes(frequency)) {
      return res
        .status(400)
        .json({ error: "Fréquence invalide (daily, weekly, monthly)" });
    }
...
router.put("/:id", authenticate, validateUUID("id"), async (req, res) => {
  try {
    // Vérifier l'accès
    const existing = await recurringService.getRecurringOrderById(
      req.params.id,
      req.user.organization_id,
    );
    if (!existing) {
    ...
    const order = await recurringService.updateRecurringOrder(
      req.params.id,
      req.user.organization_id,
      req.body,
    );
...
router.delete("/:id", authenticate, validateUUID("id"), async (req, res) => {
  try {
    ...
    await recurringService.deleteRecurringOrder(
      req.params.id,
      req.user.organization_id,
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const order = await recurringService.toggleRecurringOrder(
        req.params.id,
        req.user.organization_id,
      );
...
router.get("/admin/all", authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const orders = await recurringService.getAllRecurringOrders(
      req.user.organization_id,
      options,
    );
    res.json({ success: true, data: orders });
