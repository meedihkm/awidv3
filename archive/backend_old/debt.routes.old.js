...
router.get(
  ...
  async (req, res, next) => {
    try {
      // Si l'utilisateur est un livreur, filtrer automatiquement par ses livraisons
      const delivererId =
...
// Accessible par : admin
// ─────────────────────────────────────────────────────
router.get("/debt-payments", authorize(["admin"]), async (req, res, next) => {
  try {
